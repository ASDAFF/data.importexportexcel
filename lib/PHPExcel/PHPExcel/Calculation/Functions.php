...
/**
 * KDAPHPExcel_Calculation_Functions
 *
 * @category	KDAPHPExcel
 * @package		KDAPHPExcel_Calculation
 * @copyright	Copyright (c) 2006 - 2013 KDAPHPExcel (http://www.codeplex.com/KDAPHPExcel)
 */
class KDAPHPExcel_Calculation_Functions {
	 ...
	 *													KDAPHPExcel_Calculation_Functions::COMPATIBILITY_EXCEL			'Excel'
	 *													KDAPHPExcel_Calculation_Functions::COMPATIBILITY_GNUMERIC		'Gnumeric'
	 *													KDAPHPExcel_Calculation_Functions::COMPATIBILITY_OPENOFFICE	'OpenOfficeCalc'
	 ...
	 *												Permitted values are:
	 *													KDAPHPExcel_Calculation_Functions::RETURNDATE_PHP_NUMERIC		'P'
	 *													KDAPHPExcel_Calculation_Functions::RETURNDATE_PHP_OBJECT		'O'
	 *													KDAPHPExcel_Calculation_Functions::RETURNDATE_EXCEL			'E'
	 * @return	 boolean							Success or failure
	 */
	...
	public static function isMatrixValue($idx) {
		return ((kda_substr_count($idx,'.') <= 1) || (preg_match('/\.[A-Z]/',$idx) > 0));
	}


	public static function isValue($idx) {
		return (kda_substr_count($idx,'.') == 0);
	}


	public static function isCellValue($idx) {
		return (kda_substr_count($idx,'.') > 1);
	}


	public static function _ifCondition($condition) {
		$condition	= KDAPHPExcel_Calculation_Functions::flattenSingleValue($condition);
		if (!isset($condition{0}))
			$condition = '=""';
		if (!in_array($condition{0},array('>', '<', '='))) {
			if (!is_numeric($condition)) { $condition = KDAPHPExcel_Calculation::_wrapResult(strtoupper($condition)); }
			return '='.$condition;
		} else {
			preg_match('/([<>=]+)(.*)/',$condition,$matches);
			list(,$operator,$operand) = $matches;
			if (!is_numeric($operand)) { $operand = KDAPHPExcel_Calculation::_wrapResult(strtoupper($operand)); }
	...
	public static function VERSION() {
		return 'KDAPHPExcel 1.7.9, 2013-06-02';
	}	//	function VERSION()
