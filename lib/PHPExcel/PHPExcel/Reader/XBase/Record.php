...
class Record
{
    ...
    public function __construct(Table $table, $recordIndex, $rawData = false)
    {
		...
		if ($rawData && $this->table->strlen($rawData) > 0) {
            $this->inserted = false;
            $this->deleted = (ord($rawData[0]) != '32');

            foreach ($table->getColumns() as $column) {
                //$this->choppedData[$column->getName()] = substr($rawData, $column->getBytePos(), $column->getDataLength());
				$this->choppedData[$column->getName()] = $this->table->substr($rawData, $column->getBytePos(), $column->getDataLength());
            }
        } else {
            $this->inserted = true;
            $this->deleted = false;
    ...
    public function forceGetString($columnName)
    {
		...
		$fileEncoding = $this->table->getEncoding();
		$siteEncoding = $this->table->getSiteEncoding();
		/*if($siteEncoding!=$fileEncoding)
		{
			 $data = \Bitrix\Main\Text\Encoding::convertEncoding( $data, $fileEncoding, $siteEncoding);
		}*/
		if(ToLower($fileEncoding)!='utf-8')
		{
			 $data = \Bitrix\Main\Text\Encoding::convertEncoding( $data, $fileEncoding, 'utf-8');
		}
    ...
    public function getDateTime($columnName)
    {
		...
		$buf = unpack('i', $this->table->substr($raw, 0, 4));
        $intdate = $buf[1];
        //$buf = unpack('i', substr($raw, 4, 4));
		$buf = unpack('i', $this->table->substr($raw, 4, 4));
        $inttime = $buf[1];
    ...
    public function getMemo($columnName)
    {
		...
		if($data && $this->table->strlen($data) == 2) {
    ...
    public function setString($columnObj, $value)
    {
		...
		if($siteEncoding!=$fileEncoding)
		{
			 $value = \Bitrix\Main\Text\Encoding::convertEncoding($value, $siteEncoding, $fileEncoding);
		}
    ...
    public function forceSetString($columnObj, $value)
    {
		...
		$this->choppedData[$columnObj->getName()] = str_pad($this->table->substr($value, 0, $columnObj->getDataLength()), $columnObj->getDataLength(), " ");
    }
