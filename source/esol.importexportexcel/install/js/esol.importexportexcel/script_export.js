...
var EList = {
	...
	ShowListSettings: function(btn)
	{
		...
		var dialog = new BX.CAdminDialog({
			'title':'',
			'content_url':'/bitrix/admin/esol_import_excel_list_settings.php',
			'content_post': post,
			'width':'900',
			'height':'400',
			'resizable':true});
	...
	{
		var copySettings = ((typeof event == 'object') && event.ctrlKey);
		...
		if(copySettings)
		{
			...
			var div = $('<div></div>');
			$('input[name^="SETTINGS["], input[name^="EXTRASETTINGS["]', parent).not('[name^="SETTINGS[FILTER]"], [type="checkbox"]:not(:checked)').each(function(){
			...
			if(url.length > 0)
			{
				...
				$.ajax({
					...
					success: function(data){
						...
						var id = 'bx24_folder_id';
						var inp = $('#'+id);
						var name = inp.attr('name');
						var value = inp.val();
						var select = $('<select id="'+id+'"></select>');
						select.attr('name', name);
						var options = [], storageId = 0;
						for(var i=0; i<data.result.length; i++)
						{
							if(data.result[i].ENTITY_TYPE!='common') continue;
							options.push($('<option value="storage_'+data.result[i].ID+'">'+data.result[i].NAME+'</option>'));
							storageId = data.result[i].ID;
						}
						/*for(var i=0; i<data.result.length; i++)
						{
							if(data.result[i].ENTITY_TYPE!='group') continue;
							options.push($('<option value="group_'+data.result[i].ID+'">'+data.result[i].NAME+'</option>'));
						}*/
						for(var i=0; i<options.length; i++)
						{
							if(options[i].attr('value')==value) options[i].attr('selected', true);
							select.append(options[i]);
						}
						$.ajax({
							url: url+'disk.storage.getchildren/',
							dataType: 'json',
							data: {'id': storageId, 'filter': {'TYPE': 'folder'}},
							success: function(data){
								if(typeof data!='object' || !data.result) return;
								for(var i=0; i<data.result.length; i++)
								{
									options.push($('<option value="folder_'+data.result[i].ID+'">.'+data.result[i].NAME+'</option>'));
								}
								for(var i=0; i<options.length; i++)
								{
									if(options[i].attr('value')==value) options[i].attr('selected', true);
...
var EImport = {
	...
	GetStatus: function()
	{
		...
		$.ajax({
			type: "GET",
			url: '/upload/tmp/'+kdaIEModuleName+'/export/'+this.pid+'.txt?hash='+(new Date()).getTime(),
			success: function(data){
				var finish = false;
	...
	OnLoad: function(data)
	{
		...
		if(data.indexOf('{')!=0)
		{
			...
			var obj = this;
			setTimeout(function(){obj.SendData();}, 2000);
			return true;
		}
		eval('var result = '+data+';');
		if(typeof result == 'object')
