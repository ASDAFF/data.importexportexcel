...
if($USER->IsAdmin())
{
	...
	$tabControl->Begin();
	?>
	<form method="POST" action="<?echo $APPLICATION->GetCurPage()?>?lang=<?echo LANGUAGE_ID?>&mid_menu=1&mid=<?=$module_id?>" name="esol_importexportexcel_settings">
	...
	<tr>
		...
		<td>
			<input type="text" name="SETTINGS[IMAGES_PATH]" value="<?echo htmlspecialcharsex(COption::GetOptionString($module_id, 'IMAGES_PATH', ''))?>" size="35">
		</td>
	</tr>
	...
	<tr>
		...
		<td>
			<label><input type="radio" name="SETTINGS[NOTIFY_MODE]" value="NONE" <?if(COption::GetOptionString($module_id, 'NOTIFY_MODE', 'NONE')=='NONE') echo 'checked';?>> <? echo Loc::getMessage('KDA_IE_OPTIONS_NOTIFY_MODE_NONE'); ?></label><br>
			<label><input type="radio" name="SETTINGS[NOTIFY_MODE]" value="CRON" <?if(COption::GetOptionString($module_id, 'NOTIFY_MODE', 'NONE')=='CRON') echo 'checked';?>> <? echo Loc::getMessage('KDA_IE_OPTIONS_NOTIFY_MODE_CRON'); ?></label><br>
			<label><input type="radio" name="SETTINGS[NOTIFY_MODE]" value="ALL" <?if(COption::GetOptionString($module_id, 'NOTIFY_MODE', 'NONE')=='ALL') echo 'checked';?>> <? echo Loc::getMessage('KDA_IE_OPTIONS_NOTIFY_MODE_ALL'); ?></label>
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td>
			<label><input type="radio" name="SETTINGS[DISCOUNT_MODE]" value="SPLIT" <?if(COption::GetOptionString($module_id, 'DISCOUNT_MODE', 'SPLIT')=='SPLIT') echo 'checked';?>> <? echo Loc::getMessage('KDA_IE_OPTIONS_DISCOUNT_MODE_SPLIT'); ?></label><br>
			<label><input type="radio" name="SETTINGS[DISCOUNT_MODE]" value="JOIN" <?if(COption::GetOptionString($module_id, 'DISCOUNT_MODE', 'SPLIT')=='JOIN') echo 'checked';?>> <? echo Loc::getMessage('KDA_IE_OPTIONS_DISCOUNT_MODE_JOIN'); ?></label><br>
		</td>
	</tr>
