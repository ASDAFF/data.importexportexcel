...
class CKDAImportUtils {
	...
	public static function SaveStat()
	{
		...
		if(0 && class_exists('\Bitrix\Main\Web\HttpClient') && is_callable(array('CUpdateClientPartner', 'GetUpdatesList')))
		{
			...
			$client = new \Bitrix\Main\Web\HttpClient(array('socketTimeout'=>3));
	...
	public static function MakeFileArray($path, $maxTime = 0)
	{
					...
					if($download)
					{
						...
						elseif((!$ext || $ext=='php' || $ext=='htm' || $ext=='html') && $hct && strpos($hct, 'text/html')!==false && strpos(ToLower($path), 'htm')!==false)
						{
							if(preg_match('/charset=(.+)(;|$)/Uis', $hct, $m))
							{
								...
								{
									file_put_contents($temp_path, \Bitrix\Main\Text\Encoding::convertEncoding(file_get_contents($temp_path), $fileEncoding, $siteEncoding));
								}
							}
							$temp_path = self::ReplaceFile($temp_path, $temp_path.'.html');
						}
		...
		if(in_array($arFile['type'], array('application/zip', 'application/gzip', 'application/x-gzip', 'application/rar', 'application/x-rar', 'application/octet-stream')) && !in_array($ext, $arExt))
		{
	...
	public static function RemoveTmpFiles($maxTime = 5)
	{
		...
		$tmpDir = $docRoot.'/upload/tmp/'.static::$moduleId.'/import/';
		$arOldDirs = array();
		...
		$tmpDir = $docRoot.'/upload/tmp/'.static::$moduleId.'/import/_archives/';
		if(file_exists($tmpDir) && ($dh = opendir($tmpDir))) 
		{
			while(($file = readdir($dh)) !== false) 
			{
