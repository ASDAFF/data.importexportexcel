...
/**
 * KDAPHPExcel_Style_Color
 *
 * @category   KDAPHPExcel
 * @package	KDAPHPExcel_Style
 * @copyright  Copyright (c) 2006 - 2013 KDAPHPExcel (http://www.codeplex.com/KDAPHPExcel)
 */
class KDAPHPExcel_Style_Color extends KDAPHPExcel_Style_Supervisor implements KDAPHPExcel_IComparable
{
	/* Colors */
	...
	/**
	 * Create a new KDAPHPExcel_Style_Color
	 *
	 ...
	 *										its ramifications are
	 */
	public function __construct($pARGB = KDAPHPExcel_Style_Color::COLOR_BLACK, $isSupervisor = FALSE, $isConditional = FALSE)
	{
		//	Supervisor?
	 ...
	 *
	 * <code>
	 * $objKDAPHPExcel->getActiveSheet()->getStyle('B2')->getFont()->getColor()->applyFromArray( array('rgb' => '808080') );
	 * </code>
	 *
	 ...
	 * @return KDAPHPExcel_Style_Color
	 */
	public function setARGB($pValue = KDAPHPExcel_Style_Color::COLOR_BLACK) {
		if ($pValue == '') {
			$pValue = KDAPHPExcel_Style_Color::COLOR_BLACK;
		}
		if ($this->_isSupervisor) {
	...
	public function getRealRGB() {
		if ($this->_isSupervisor) {
			return $this->getSharedComponent()->getRGB();
		}
		$len = strlen($this->_argb);
		if($len==3 || $len==6) return $this->_argb;
		else return substr($this->_argb, 2);
	}
	...
	public static function setIndexedColor($indexedColors) {
		self::$_indexedColors = $indexedColors;
	}
	...
	public static function indexedColor($pIndex, $background=FALSE) {
		...
		if (array_key_exists($pIndex, self::$_indexedColors)) {
			return new KDAPHPExcel_Style_Color(self::$_indexedColors[$pIndex]);
		}

		if ($background) {
			return new KDAPHPExcel_Style_Color('FFFFFFFF');
		}
		return new KDAPHPExcel_Style_Color('FF000000');
	}
