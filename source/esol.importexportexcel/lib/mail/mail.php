		...
		if($this->CheckParams($tmpdir))
		{
			...
			while(!empty($arFolders) && !$fid)
			{
				...
				if(!empty($mailsIds))
				{
					...
					{
						$mailId = $mailsIds[$i];
						if(($arMailFile = $this->imap->getMessageFile($folder, $mailId, $this->params['FILENAME'], array('txt', 'csv', 'xls', 'xlsx', 'xlsm', 'zip', 'gz', 'tgz', 'rar'), ($error='')))!==false)
						{
							if(!$this->params['LAST_DATE'] || $arMailFile['DATE']!=$this->params['LAST_DATE'])
							{
								...
								$arFile = \CKDAImportUtils::MakeFileArray($tmpdir.$fn);
