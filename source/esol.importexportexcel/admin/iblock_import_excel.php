...
if ($REQUEST_METHOD == "POST" && $STEP > 1 && check_bitrix_sessid())
{
		...
		if (strlen($strError) <= 0)
		{
			if($STEP==2 && !$notRewriteFile)
			{
				if((!isset($_FILES["DATA_FILE"]) || !$_FILES["DATA_FILE"]["tmp_name"]) && (!isset($_POST['DATA_FILE']) || is_numeric($_POST['DATA_FILE'])))
				{
					...
					{
						$fileId = \Bitrix\KdaImportexcel\SMail::GetNewFile($SETTINGS_DEFAULT['EMAIL_DATA_FILE']);
						if($fileId > 0)
						{
	...
	if($ACTION == 'SHOW_FULL_LIST')
	{
		try{
			...
			$arWorksheets = CKDAImportExcel::GetPreviewData($DATA_FILE_NAME, $SHOW_FIRST_LINES, $pparams, $COUNT_COLUMNS);
		}catch(Exception $ex){
			$APPLICATION->RestartBuffer();
			ob_end_clean();
	...
	if($ACTION == 'SHOW_REVIEW_LIST')
	{
		...
		try{
			$pparams = $SETTINGS_DEFAULT;
			if(isset($SETTINGS['CSV_PARAMS'])) $pparams['CSV_PARAMS'] = $SETTINGS['CSV_PARAMS'];
			$arWorksheets = CKDAImportExcel::GetPreviewData($DATA_FILE_NAME, $SHOW_FIRST_LINES, $pparams);
			if($SETTINGS_DEFAULT['AUTO_CREATION_PROPERTIES']=='Y')
			{
				$oProfile = new CKDAImportProfile();
						...
						<?if(count($worksheet['lines']) > 0){?>
							...
							if(is_array($EXTRASETTINGS[$k]))
							{
								foreach($EXTRASETTINGS[$k] as $k2=>$v2)
								{
									...
									{
										?><a href="javascript:void(0)" id="field_settings_<?echo $k;?>_<?echo $k2;?>" onclick="EList.ShowFieldSettings(this);"><input type="hidden" name="EXTRASETTINGS[<?echo $k;?>][<?echo $k2;?>]" value=""><script>EList.SetExtraParams("field_settings_<?echo $k;?>_<?echo $k2;?>", <?echo CUtil::PhpToJSObject($v2);?>)</script><?
									}
								}
							}
						}?>
						...
						<div class="addsettings">
							...
							<div class="addsettings_inner">
								<table class="additional">
									...
									<?
									$fileExt = CKDAImportUtils::GetFileExtension($DATA_FILE_NAME);
									$changeCsvParams = (bool)($SETTINGS['CSV_PARAMS']['CHANGE']=='Y');
									if(ToLower($fileExt)=='csv')
									{
									?>
										<tr>
											<td><?echo GetMessage("KDA_IE_CHANGE_CSV_PARAMS"); ?>:</td>
											<td>
...
$aMenu = array(
	array(
		"TEXT"=>GetMessage("KDA_IE_MENU_HELP"),
		"TITLE"=>GetMessage("KDA_IE_MENU_HELP"),
...
<form method="POST" action="<?echo $sDocPath ?>?lang=<?echo LANG ?>" ENCTYPE="multipart/form-data" name="dataload" id="dataload" class="kda-ie-s1-form">
	...
	?>
		...
		<tr>
			<td colspan="2" align="center">
				...
				<a href="javascript:void(0)" onclick="EProfile.OpenMissignElementFields(this)"><?echo GetMessage("KDA_IE_ELEMENT_MISSING_SET_FIELDS"); ?></a>
			</td>
		</tr>
		...
		<tr>
			<td><?echo GetMessage("KDA_IE_REMOVE_COMPOSITE_CACHE"); ?>:</td>
			<td>
				<input type="checkbox" name="SETTINGS_DEFAULT[REMOVE_COMPOSITE_CACHE]" value="Y" <?if($SETTINGS_DEFAULT['REMOVE_COMPOSITE_CACHE']=='Y'){echo 'checked';}?>>
			</td>
		</tr>
		 ...
		 <table width="100%"><tr><td width="50%">
			...
			<div class="detail_status">
				<?echo CAdminMessage::ShowMessage(array(
					...
					.GetMessage("KDA_IE_SU_ELEMENT_CHANGED").' <b id="element_changed_line">0</b><br>'
					.($SETTINGS_DEFAULT['ONLY_DELETE_MODE']=='Y' ? (GetMessage("KDA_IE_SU_ELEMENT_DELETED").' <b id="element_removed_line">0</b><br>') : '')
					.(!empty($SETTINGS_DEFAULT['ELEMENT_UID_SKU']) ? (GetMessage("KDA_IE_SU_SKU_ADDED").' <b id="sku_added_line">0</b><br>') : '')
					.(!empty($SETTINGS_DEFAULT['ELEMENT_UID_SKU']) ? (GetMessage("KDA_IE_SU_SKU_UPDATED").' <b id="sku_updated_line">0</b><br>') : '')
					.(!empty($SETTINGS_DEFAULT['ELEMENT_UID_SKU']) ? (GetMessage("KDA_IE_SU_SKU_CHANGED").' <b id="sku_changed_line">0</b><br>') : '')
					.GetMessage("KDA_IE_SU_SECTION_ADDED").' <b id="section_added_line">0</b><br>'
					.GetMessage("KDA_IE_SU_SECTION_UPDATED").' <b id="section_updated_line">0</b><br>'
					.($SETTINGS_DEFAULT['CELEMENT_MISSING_DEACTIVATE']=='Y' ? (GetMessage("KDA_IE_SU_HIDED").' <b id="killed_line">0</b><br>') : '')
					.($SETTINGS_DEFAULT['OFFER_MISSING_DEACTIVATE']=='Y' ? (GetMessage("KDA_IE_SU_OFFER_HIDED").' <b id="offer_killed_line">0</b><br>') : '')
					.($SETTINGS_DEFAULT['CELEMENT_MISSING_TO_ZERO']=='Y' ? (GetMessage("KDA_IE_SU_ZERO_STOCK").' <b id="zero_stock_line">0</b><br>') : '')
					.($SETTINGS_DEFAULT['OFFER_MISSING_TO_ZERO']=='Y' ? (GetMessage("KDA_IE_SU_OFFER_ZERO_STOCK").' <b id="offer_zero_stock_line">0</b><br>') : '')
					.($SETTINGS_DEFAULT['CELEMENT_MISSING_REMOVE_ELEMENT']=='Y' ? (GetMessage("KDA_IE_SU_REMOVE_ELEMENT").' <b id="old_removed_line">0</b><br>') : '')
					.($SETTINGS_DEFAULT['OFFER_MISSING_REMOVE_ELEMENT']=='Y' ? (GetMessage("KDA_IE_SU_OFFER_REMOVE_ELEMENT").' <b id="offer_old_removed_line">0</b><br>') : '')
