			...
			<td class="kda-ie-settings-margin-container" colspan="2">
				...
				foreach($arVals as $k=>$v)
				{
					echo '<div class="kda-ie-settings-conversion" '.(!$showCondition ? 'style="display: none;"' : '').'>'.
							...
							'</select> '.
							'<input type="text" name="'.$fName.'[FROM][]" value="'.htmlspecialcharsex($v['FROM']).'"> '.
							...
							'<input class="choose_val" value="..." type="button" onclick="ESettings.ShowChooseVal(this, '.intval($_REQUEST['count_cols']).')">'.
							'<a href="javascript:void(0)" onclick="ESettings.RemoveConversion(this)" title="'.GetMessage("KDA_IE_SETTINGS_DELETE").'" class="delete"></a>'.
						 '</div>';
				}
				?>
				<a href="javascript:void(0)" onclick="ESettings.AddConversion(this)"><?echo GetMessage("KDA_IE_SETTINGS_CONVERSION_ADD_VALUE");?></a>
			</td>
		</tr>
