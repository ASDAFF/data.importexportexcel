...
class MailHeader
{
	...
	function ConvertHeader($encoding, $type, $str, $charset)
	{
		...
		$str = \Bitrix\KdaImportexcel\MailUtil::ConvertCharset($str, $encoding, $charset);
	...
	function DecodeHeader($str, $charset_to, $charset_document)
	{
		...
		if(!preg_match("'=\?(.*)\?(B|Q)\?(.*)\?='i", $str))
		{
			if(strlen($charset_document)>0 && $charset_document!=$charset_to)
				$str = \Bitrix\KdaImportexcel\MailUtil::ConvertCharset($str, $charset_document, $charset_to);
		}
		else
		{
			$str = preg_replace_callback(
				"'=\?(.*?)\?(B|Q)\?(.*?)\?='i",
				create_function('$m', "return \Bitrix\KdaImportexcel\MailHeader::ConvertHeader(\$m[1], \$m[2], \$m[3], '".AddSlashes($charset_to)."');"),
				$str
			);
		}
	...
	function Parse($message_header, $charset)
	{
		...
		if (strlen($cd) > 0)
		{
			...
			else if (preg_match("'filename\*=([^;]+)'i", $cd, $res))
			{
				...
				$this->filename = \Bitrix\KdaImportexcel\MailUtil::ConvertCharset(rawurldecode($fnstr), $fncharset, $charset);
			}
		}
