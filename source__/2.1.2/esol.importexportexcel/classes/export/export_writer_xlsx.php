...
class CKDAExportExcelWriterXlsx {
	...
	public function Save()
	{
		...
		foreach($this->arListIndexes as $indexKey=>$listIndex)
		{
			...
			for($i=1; $i<=$this->totalCols; $i++)
			{
				...
				if((int)$this->fparams[$i-1]['DISPLAY_WIDTH'] > 0) $colWidth = (int)$this->fparams[$i-1]['DISPLAY_WIDTH'];
			...
			while(!feof($handle)) 
			{
				...
				else
				{
					...
					foreach($arFields as $k=>$field)
					{
						if($this->fparams[$k]['INSERT_PICTURE']!='Y' || !$this->ee->IsPictureField($field)) continue;
					...
					foreach($arVals as $valIndex=>$arValue)
					{
						...
						foreach($arFields as $k=>$field)
						{
							...
							if($this->fparams[$k]['INSERT_PICTURE']=='Y' && $this->ee->IsPictureField($field))
							{
					...
					foreach($arImgVals as $k=>$arImgs)
					{
						...
						foreach($arImgs as $arImg)
						{
							...
							fwrite($this->drawingsHandler, '<xdr:twoCellAnchor>'.
								'<xdr:from><xdr:col>'.$k.'</xdr:col><xdr:colOff>'.$leftOffset.'</xdr:colOff><xdr:row>'.$currentRow.'</xdr:row><xdr:rowOff>'.$this->imagePixel.'</xdr:rowOff></xdr:from>'.
								'<xdr:to><xdr:col>'.$k.'</xdr:col><xdr:colOff>'.($leftOffset + $width).'</xdr:colOff><xdr:row>'.($currentRow+$rowOffset).'</xdr:row><xdr:rowOff>'.($height + $this->imagePixel).'</xdr:rowOff></xdr:to>'.
								'<xdr:pic>'.
									'<xdr:nvPicPr>'.
									(
										strlen($link) > 0 ? 
    ...
    public function openExcelWriter()
    {
		...
		$this->styleCellXfs = array(
			'<xf numFmtId="0" fontId="1" fillId="0" borderId="0" xfId="0" applyAlignment="1">'.$this->getAlignment().'</xf>'
		);
    }
    ...
    public function openWorkSheet($listIndex)
    {
		...
		for($i=1; $i<=$cols; $i++)
		{
			...
			if((int)$this->fparams[$i-1]['DISPLAY_WIDTH'] > 0) $width = (int)$this->fparams[$i-1]['DISPLAY_WIDTH'];
	...
	public function setCurrentStyle(&$colHeight, $arStyles, $saveStyle = true)
	{
		...
		if(!empty($arStyles))
		{
			...
			{
				if(!in_array($k, array('FONT_FAMILY', 'FONT_SIZE', 'FONT_COLOR', 'STYLE_BOLD', 'STYLE_ITALIC', 'STYLE_UNDERLINE', 'BACKGROUND_COLOR', 'ROW_HEIGHT', 'TEXT_ALIGN', 'VERTICAL_ALIGN', 'BORDER_STYLE', 'BORDER_STYLE_SIDE', 'BORDER_COLOR', 'INDENT', 'NUMBER_FORMAT')))
				{
					unset($arStyles[$k]);
				}
			}
			...
			if(!isset($this->arStyleIds[$hash]))
			{
				...
				if($fontId > 0 || $fillId > 0 || $setAlignment || $borderId > 0 || $arStyles['INDENT'] || $arStyles['NUMBER_FORMAT'])
				{
					$numFmtId = 'numFmtId="0"';
					if((int)$arStyles['NUMBER_FORMAT'] > 0) $numFmtId = 'numFmtId="'.(int)$arStyles['NUMBER_FORMAT'].'" applyNumberFormat="1"';
					$this->styleCellXfs[] = '<xf '.$numFmtId.' fontId="'.$fontId.'" fillId="'.$fillId.'" '.($borderId > 0 ? 'borderId="'.$borderId.'" applyBorder="1"' : 'borderId="0"').' xfId="'.count($this->styleCellXfs).'"'.($fontId > 0 ? ' applyFont="1"' : '').($fillId > 0 ? ' applyFill="1"' : '').' applyAlignment="1">'.$this->getAlignment($arStyles).'</xf>';
    ...
    public function writeStringCell($value, $colspan=1, $arStyles=array(), $isData=false, $cellType='')
    {
		...
		if((int)$arStyles['NUMBER_FORMAT'] > 0 && strlen($cellType)==0) $cellType = 'NUMBER';
        $this->curCel++;
		$cell = $this->curCel;
        if (1) {
			...
			if($this->fparams[$cell - 1]['MAKE_DROPDOWN']=='Y' && $isData && $colspan < 2)
			{
		...
		if(!empty($textRows))
		{
			foreach($textRows as $k=>$v)
			{
				...
				if(preg_match('/^\[\[(\d+)\]\]$/', $v, $m))
				{
					...
					$arFileOrig = CFile::GetFileArray($fileId);
    ...
    public function closeExcelWriter($break = false)
    {
		...
		if(class_exists('ZipArchive') && ($zipObj = new ZipArchive()) && $zipObj->open($this->outputFile, ZipArchive::OVERWRITE)===true)
		{
