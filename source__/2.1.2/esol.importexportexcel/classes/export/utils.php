...
class CKDAExportUtils {
	...
	public static function ShowFilter($sTableID, $listIndex, $SETTINGS, $SETTINGS_DEFAULT)
	{
		...
		?>
			...
			<tr>
				...
				<td><?echo CalendarPeriod("SETTINGS[FILTER][".$listIndex."][find_el_date_active_from_from]", htmlspecialcharsex($arFields['find_el_date_active_from_from']), "SETTINGS[FILTER][".$listIndex."][find_el_date_active_from_to]", htmlspecialcharsex($arFields['find_el_date_active_from_to']), "dataload")?></td>
			</tr>

			<tr>
				<td><?echo Loc::getMessage("KDA_EE_EL_A_ACTTO")?>:</td>
				<td><?echo CalendarPeriod("SETTINGS[FILTER][".$listIndex."][find_el_date_active_to_from]", htmlspecialcharsex($arFields['find_el_date_active_to_from']), "SETTINGS[FILTER][".$listIndex."][find_el_date_active_to_to]", htmlspecialcharsex($arFields['find_el_date_active_to_to']), "dataload")?></td>
			</tr>
	...
	public static function RemoveTmpFiles($maxTime = 5)
	{
		...
		$tmpDir = $docRoot.'/upload/tmp/'.static::$moduleId.'/export/';
		$arOldDirs = array();
		$arActDirs = array();
