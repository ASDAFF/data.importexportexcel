...
var EList = {
		...
		$('select[name^="FIELDS_LIST["]', gParent).each(function(){
			...
			$('.kda-ie-field-select', parent).each(function(index){
				...
				if(ind)
				{
					...
					$('a.field_insert', this).attr('title', arVals[ind]+' ('+admKDAMessages.insertFindField+')')
						.attr('data-value', ind)
	...
	ShowFieldSettings: function(btn)
	{
		...
		var dialog = new BX.CAdminDialog({
			'title':admKDAMessages.popupFieldSettingsTitle + (input2.val() ? ' "'+input2.val()+'"' : ''),
...
var EImport = {
	...
	GetStatus: function()
	{
		...
		$.ajax({
			type: "GET",
			url: '/upload/tmp/'+kdaIEModuleName+'/import/'+this.pid+'_highload.txt?hash='+(new Date()).getTime(),
			success: function(data){
				var finish = false;
