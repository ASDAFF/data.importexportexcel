...
function ShowTblLine($data, $list, $line, $checked = true)
{
	?><tr><td class="line-settings">
		...
		?><td <?echo $parentStyle;?>><div class="cell" <?echo $parentStyle;?>><div class="cell_inner" <?echo $style;?> <?echo $dataStyle;?>><?echo $row['VALUE'];?></div></div></td><?
		}
	?></tr><?
}
/////////////////////////////////////////////////////////////////////
...
if ($REQUEST_METHOD == "POST" && $STEP > 1 && check_bitrix_sessid())
{
	...
	if ($STEP > 1)
	{
		...
		{
			if($_POST["EXT_DATA_FILE"] && $STEP==2 && (!isset($_FILES["DATA_FILE"]) || !$_FILES["DATA_FILE"]["tmp_name"]) && (!isset($_POST['DATA_FILE']) || is_numeric($_POST['DATA_FILE'])))
			{
				$_POST['DATA_FILE'] = $_POST["EXT_DATA_FILE"];
			}
		...
		if (strlen($strError) <= 0)
		{
			...
			if(!in_array(ToLower(GetFileExtension($DATA_FILE_NAME)), array('txt', 'csv', 'xls', 'xlsx', 'xlsm')))
			{
	...
	if($ACTION == 'SHOW_FULL_LIST')
	{
		...
		$_SESSION = $sess;
		try{
			$arWorksheets = CKDAImportExcelHighload::GetPreviewData($DATA_FILE_NAME, $SHOW_FIRST_LINES, $SETTINGS_DEFAULT, $COUNT_COLUMNS);
		}catch(Exception $ex){
			$APPLICATION->RestartBuffer();
			ob_end_clean();
	...
	if($ACTION == 'SHOW_REVIEW_LIST')
	{
		...
		try{
			$arWorksheets = CKDAImportExcelHighload::GetPreviewData($DATA_FILE_NAME, $SHOW_FIRST_LINES, $SETTINGS_DEFAULT);
		}catch(Exception $ex){
			$APPLICATION->RestartBuffer();
			ob_end_clean();
			echo GetMessage("KDA_IE_ERROR").$ex->getMessage();
	...
	if($ACTION == 'DO_IMPORT')
	{
		...
		session_write_close();
		$_SESSION = $sess;
		$ie = new CKDAImportExcelHighload($DATA_FILE_NAME, $params, $EXTRASETTINGS, $stepparams, $PROFILE_ID);
		$arResult = $ie->Import();
		$APPLICATION->RestartBuffer();
		ob_end_clean();
...
?>
	...
	?>
		...
		<tr>
			...
			<td>
				<input type="checkbox" name="SETTINGS_DEFAULT[ONLY_UPDATE_MODE]" value="Y" <?if($SETTINGS_DEFAULT['ONLY_UPDATE_MODE']=='Y'){echo 'checked';}?> onchange="EProfile.RadioChb(this, 'SETTINGS_DEFAULT[ONLY_CREATE_MODE]')">
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<input type="checkbox" name="SETTINGS_DEFAULT[ONLY_CREATE_MODE]" value="Y" <?if($SETTINGS_DEFAULT['ONLY_CREATE_MODE']=='Y'){echo 'checked';}?> onchange="EProfile.RadioChb(this, 'SETTINGS_DEFAULT[ONLY_UPDATE_MODE]')">
			</td>
		</tr>
		 ...
		 <table width="100%"><tr><td width="50%">
			...
			<div class="detail_status">
				<?echo CAdminMessage::ShowMessage(array(
					...
					.($SETTINGS_DEFAULT['ELEMENT_MISSING_REMOVE_ELEMENT']=='Y' ? (GetMessage("KDA_IE_SU_REMOVE_ELEMENT").' <b id="element_removed_line">0</b><br>') : '')
...
var admKDAMessages = {
	'insertFindField': '<?echo GetMessage("KDA_IE_INSERT_FIND_FIELD");?>',
	'popupFieldSettingsTitle': '<?echo GetMessage("KDA_IE_POPUP_FIELD_SETTINGS_TITLE");?>'
};
...
<?if ($STEP < 2): 
	...
	{
		?>
		$('#bx_file_data_file_cont .adm-input-file-name').attr('href', '<?echo htmlspecialcharsex($SETTINGS_DEFAULT["EXT_DATA_FILE"])?>').text('<?echo $SETTINGS_DEFAULT["EXT_DATA_FILE"]?>');<?
	}
?>
