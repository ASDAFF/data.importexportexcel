...
<form action="" method="post" enctype="multipart/form-data" name="field_settings">
	...
	<table width="100%">
		...
		<?if(preg_match('/^ICAT_PRICE\d+_PRICE_DISCOUNT$/', $field)){?>
			<tr>
		...
		<?if($field=="IE_SECTION_PATH"){?>
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					...
					eval('$val = $P'.$fNameEval.';');
					?>
					<input type="text" name="<?=$fName?>" value="<?=htmlspecialcharsex($val)?>" placeholder="<?echo GetMessage("KDA_EE_SETTINGS_SECTION_PATH_SEPARATOR_PLACEHOLDER");?>">
				</td>
			</tr>
		<?}?>
