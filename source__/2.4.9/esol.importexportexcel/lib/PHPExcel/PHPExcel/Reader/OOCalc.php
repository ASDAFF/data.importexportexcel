...
/**
 * KDAPHPExcel_Reader_OOCalc
 *
 * @category	KDAPHPExcel
 * @package		KDAPHPExcel_Reader
 * @copyright	Copyright (c) 2006 - 2013 KDAPHPExcel (http://www.codeplex.com/KDAPHPExcel)
 */
class KDAPHPExcel_Reader_OOCalc extends KDAPHPExcel_Reader_Abstract implements KDAPHPExcel_Reader_IReader
{
	/**
	 * Formats
	 *
	...
	/**
	 * Create a new KDAPHPExcel_Reader_OOCalc
	 */
	...
	public function listWorksheetNames($pFilename)
	{
		...
		if (!$zip->open($pFilename)) {
			throw new KDAPHPExcel_Reader_Exception("Could not open " . $pFilename . " for reading! Error opening file.");
		}
			...
			foreach($officePropertyMeta as $propertyName => $propertyValue) {
				...
				switch ($propertyName) {
					...
					case 'creation-date' :
							...
							break;
					case 'user-defined' :
							$propertyValueType = KDAPHPExcel_DocumentProperties::PROPERTY_TYPE_STRING;
							foreach ($propertyValueAttributes as $key => $value) {
								if ($key == 'name') {
									...
									switch ($value) {
										case 'date'	:
											$propertyValue = KDAPHPExcel_DocumentProperties::convertProperty($propertyValue,'date');
											$propertyValueType = KDAPHPExcel_DocumentProperties::PROPERTY_TYPE_DATE;
											break;
										case 'boolean'	:
											$propertyValue = KDAPHPExcel_DocumentProperties::convertProperty($propertyValue,'bool');
											$propertyValueType = KDAPHPExcel_DocumentProperties::PROPERTY_TYPE_BOOLEAN;
											break;
										case 'float'	:
											$propertyValue = KDAPHPExcel_DocumentProperties::convertProperty($propertyValue,'r4');
											$propertyValueType = KDAPHPExcel_DocumentProperties::PROPERTY_TYPE_FLOAT;
											break;
										default :
											$propertyValueType = KDAPHPExcel_DocumentProperties::PROPERTY_TYPE_STRING;
									}
...
//									echo 'Value Type is '.$cellDataOfficeAttributes['value-type'].'<br />';
									switch ($cellDataOfficeAttributes['value-type']) {
										...
										case 'percentage' :
												...
												$formatting = KDAPHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE_00;
												break;
										case 'currency' :
												$type = KDAPHPExcel_Cell_DataType::TYPE_NUMERIC;
												$dataValue = (float) $cellDataOfficeAttributes['value'];
												...
												$formatting = KDAPHPExcel_Style_NumberFormat::FORMAT_CURRENCY_USD_SIMPLE;
												break;
										case 'float' :
												$type = KDAPHPExcel_Cell_DataType::TYPE_NUMERIC;
												$dataValue = (float) $cellDataOfficeAttributes['value'];
										...
										case 'date' :
												...
												$dataValue = KDAPHPExcel_Shared_Date::FormattedPHPToExcel($year,$month,$day,$hour,$minute,$second);
												if ($dataValue != floor($dataValue)) {
													$formatting = KDAPHPExcel_Style_NumberFormat::FORMAT_DATE_XLSX15.' '.KDAPHPExcel_Style_NumberFormat::FORMAT_DATE_TIME4;
												} else {
													$formatting = KDAPHPExcel_Style_NumberFormat::FORMAT_DATE_XLSX15;
												}
												break;
										case 'time' :
												$type = KDAPHPExcel_Cell_DataType::TYPE_NUMERIC;
												$dataValue = KDAPHPExcel_Shared_Date::PHPToExcel(strtotime('01-01-1970 '.implode(':',sscanf($cellDataOfficeAttributes['time-value'],'PT%dH%dM%dS'))));
												$formatting = KDAPHPExcel_Style_NumberFormat::FORMAT_DATE_TIME4;
												break;
									}
...
//									echo 'Formula: '.$cellDataFormula.'<br />';
									...
									foreach($temp as &$value) {
										...
										if ($tKey = !$tKey) {
											...
											$value = KDAPHPExcel_Calculation::_translateSeparator(';',',',$value,$inBraces);
										}
									}
									unset($value);
...
//									echo 'Adjusted Formula: '.$cellDataFormula.'<br />';
								...
								if ($type !== NULL) {
									for ($i = 0; $i < $colRepeats; ++$i) {
										...
										if ($type !== KDAPHPExcel_Cell_DataType::TYPE_NULL) {
											for ($rowAdjust = 0; $rowAdjust < $rowRepeats; ++$rowAdjust) {
												$rID = $rowID + $rowAdjust;
												$objKDAPHPExcel->getActiveSheet()->getCell($columnID.$rID)->setValueExplicit((($hasCalculatedValue) ? $cellDataFormula : $dataValue),$type);
												if ($hasCalculatedValue) {
//													echo 'Forumla result is '.$dataValue.'<br />';
													$objKDAPHPExcel->getActiveSheet()->getCell($columnID.$rID)->setCalculatedValue($dataValue);
												}
												if ($formatting !== NULL) {
													$objKDAPHPExcel->getActiveSheet()->getStyle($columnID.$rID)->getNumberFormat()->setFormatCode($formatting);
												} else {
													$objKDAPHPExcel->getActiveSheet()->getStyle($columnID.$rID)->getNumberFormat()->setFormatCode(KDAPHPExcel_Style_NumberFormat::FORMAT_GENERAL);
												}
												if ($hyperlink !== NULL) {
													$objKDAPHPExcel->getActiveSheet()->getCell($columnID.$rID)->getHyperlink()->setUrl($hyperlink);
												}
											}
										}
									}
								...
								if ((isset($cellDataTableAttributes['number-columns-spanned'])) || (isset($cellDataTableAttributes['number-rows-spanned']))) {
									if (($type !== KDAPHPExcel_Cell_DataType::TYPE_NULL) || (!$this->_readDataOnly)) {
										$columnTo = $columnID;
										if (isset($cellDataTableAttributes['number-columns-spanned'])) {
											$columnTo = KDAPHPExcel_Cell::stringFromColumnIndex(KDAPHPExcel_Cell::columnIndexFromString($columnID) + $cellDataTableAttributes['number-columns-spanned'] -2);
										}
										$rowTo = $rowID;
