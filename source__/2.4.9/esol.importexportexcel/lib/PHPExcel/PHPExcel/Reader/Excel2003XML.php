...
/**
 * KDAPHPExcel_Reader_Excel2003XML
 *
 * @category   KDAPHPExcel
 * @package    KDAPHPExcel_Reader
 * @copyright  Copyright (c) 2006 - 2013 KDAPHPExcel (http://www.codeplex.com/KDAPHPExcel)
 */
class KDAPHPExcel_Reader_Excel2003XML extends KDAPHPExcel_Reader_Abstract implements KDAPHPExcel_Reader_IReader
{
	/**
	 * Formats
	 *
	...
	/**
	 * Create a new KDAPHPExcel_Reader_Excel2003XML
	 */
...
//		echo 'Character Set is ',$this->_charSet,'<br />';
	...
	public function loadIntoExisting($pFilename, KDAPHPExcel $objKDAPHPExcel)
	{
		...
		$underlineStyles = array (
				KDAPHPExcel_Style_Font::UNDERLINE_NONE,
				KDAPHPExcel_Style_Font::UNDERLINE_DOUBLE,
				KDAPHPExcel_Style_Font::UNDERLINE_DOUBLEACCOUNTING,
				KDAPHPExcel_Style_Font::UNDERLINE_SINGLE,
				KDAPHPExcel_Style_Font::UNDERLINE_SINGLEACCOUNTING
			);
		$verticalAlignmentStyles = array (
				KDAPHPExcel_Style_Alignment::VERTICAL_BOTTOM,
				KDAPHPExcel_Style_Alignment::VERTICAL_TOP,
				KDAPHPExcel_Style_Alignment::VERTICAL_CENTER,
				KDAPHPExcel_Style_Alignment::VERTICAL_JUSTIFY
			);
		$horizontalAlignmentStyles = array (
				KDAPHPExcel_Style_Alignment::HORIZONTAL_GENERAL,
				KDAPHPExcel_Style_Alignment::HORIZONTAL_LEFT,
				KDAPHPExcel_Style_Alignment::HORIZONTAL_RIGHT,
				KDAPHPExcel_Style_Alignment::HORIZONTAL_CENTER,
				KDAPHPExcel_Style_Alignment::HORIZONTAL_CENTER_CONTINUOUS,
				KDAPHPExcel_Style_Alignment::HORIZONTAL_JUSTIFY
			);
		...
		if (isset($xml->CustomDocumentProperties)) {
			foreach($xml->CustomDocumentProperties[0] as $propertyName => $propertyValue) {
				...
				$propertyName = preg_replace_callback('/_x([0-9a-z]{4})_/','KDAPHPExcel_Reader_Excel2003XML::_hex2str',$propertyName);
				$propertyType = KDAPHPExcel_DocumentProperties::PROPERTY_TYPE_UNKNOWN;
				switch((string) $propertyAttributes) {
					case 'string' :
						$propertyType = KDAPHPExcel_DocumentProperties::PROPERTY_TYPE_STRING;
						$propertyValue = trim($propertyValue);
						break;
					case 'boolean' :
						$propertyType = KDAPHPExcel_DocumentProperties::PROPERTY_TYPE_BOOLEAN;
						$propertyValue = (bool) $propertyValue;
						break;
					case 'integer' :
						$propertyType = KDAPHPExcel_DocumentProperties::PROPERTY_TYPE_INTEGER;
						$propertyValue = intval($propertyValue);
						break;
					case 'float' :
						$propertyType = KDAPHPExcel_DocumentProperties::PROPERTY_TYPE_FLOAT;
						$propertyValue = floatval($propertyValue);
						break;
					case 'dateTime.tz' :
						$propertyType = KDAPHPExcel_DocumentProperties::PROPERTY_TYPE_DATE;
...
//									echo $borderStyleKey.' = '.$borderStyleValue.'<br />';
									switch ($borderStyleKey) {
										case 'LineStyle' :
												$thisBorder['style'] = KDAPHPExcel_Style_Border::BORDER_MEDIUM;
			...
			if (isset($worksheet->Table->Column)) {
				foreach($worksheet->Table->Column as $columnData) {
					...
					if (isset($columnData_ss['Index'])) {
						$columnID = KDAPHPExcel_Cell::stringFromColumnIndex($columnData_ss['Index']-1);
					}
					if (isset($columnData_ss['Width'])) {
						$columnWidth = $columnData_ss['Width'];
//						echo '<b>Setting column width for '.$columnID.' to '.$columnWidth.'</b><br />';
						$objKDAPHPExcel->getActiveSheet()->getColumnDimension($columnID)->setWidth($columnWidth / 5.4);
					}
					++$columnID;
				}
			}
...
//					echo '<b>Row '.$rowID.'</b><br />';
					...
					foreach($rowData->Cell as $cell) {
						...
						if (isset($cell_ss['Index'])) {
							$columnID = KDAPHPExcel_Cell::stringFromColumnIndex($cell_ss['Index']-1);
						}
						...
						if ((isset($cell_ss['MergeAcross'])) || (isset($cell_ss['MergeDown']))) {
							...
							if (isset($cell_ss['MergeAcross'])) {
								$columnTo = KDAPHPExcel_Cell::stringFromColumnIndex(KDAPHPExcel_Cell::columnIndexFromString($columnID) + $cell_ss['MergeAcross'] -1);
							}
							$rowTo = $rowID;
							...
							$objKDAPHPExcel->getActiveSheet()->mergeCells($cellRange);
						}
							...
							if (isset($cellData_ss['Type'])) {
								...
								switch ($cellDataType) {
									...
									*/
									...
									case 'DateTime' :
											$type = KDAPHPExcel_Cell_DataType::TYPE_NUMERIC;
											$cellValue = KDAPHPExcel_Shared_Date::PHPToExcel(strtotime($cellValue));
											break;
									case 'Error' :
...
//								echo 'FORMULA<br />';
								...
								$columnNumber = KDAPHPExcel_Cell::columnIndexFromString($columnID);
...
//
							$objKDAPHPExcel->getActiveSheet()->getCell($columnID.$rowID)->setValueExplicit((($hasCalculatedValue) ? $cellDataFormula : $cellValue),$type);
							if ($hasCalculatedValue) {
//								echo 'Formula result is '.$cellValue.'<br />';
								$objKDAPHPExcel->getActiveSheet()->getCell($columnID.$rowID)->setCalculatedValue($cellValue);
							}
...
//							echo 'Annotation: ',$annotation,'<br />';
							$objKDAPHPExcel->getActiveSheet()->getComment( $columnID.$rowID )
...
//								echo '<br />';
								if (!$objKDAPHPExcel->getActiveSheet()->cellExists($columnID.$rowID)) {
									$objKDAPHPExcel->getActiveSheet()->getCell($columnID.$rowID)->setValue(NULL);
								}
								$objKDAPHPExcel->getActiveSheet()->getStyle($cellRange)->applyFromArray($this->_styles[$style]);
							}
						}
						++$columnID;
					}
...
//							echo '<b>Setting row height to '.$rowHeight.'</b><br />';
							$objKDAPHPExcel->getActiveSheet()->getRowDimension($rowID)->setRowHeight($rowHeight);
						}
					}
	...
	private static function _convertStringEncoding($string,$charset) {
		if ($charset != 'UTF-8') {
			return KDAPHPExcel_Shared_String::ConvertEncoding($string,'UTF-8',$charset);
		}
		return $string;
	}
