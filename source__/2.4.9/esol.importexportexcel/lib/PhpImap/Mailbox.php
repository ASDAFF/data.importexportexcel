...
class Mailbox {
	...
	public function switchMailbox($imapPath = '') {
		...
		$imapStream = @imap_reopen($this->getImapStream(), $imapPath, $this->imapOptions);
		if(!$imapStream) {
	...
	public function getListingFolders() {
		...
		$inboxFolder = preg_grep('/\}INBOX/i', $folders);
		if(count($inboxFolder) > 0)
		{
			$inboxFolder = current($inboxFolder);
			$folders = array_diff($folders, array($inboxFolder));
			array_unshift($folders, $inboxFolder);
		}
		$arFolders = array();
		foreach ($folders as $key => $folder)
		{
			$origFolder = str_replace($this->imapPath, "", $folder);
